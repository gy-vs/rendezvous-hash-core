...
const NODES: readonly RendezvousNode[] = [
  { id: 'node-a', weight: 1 },
  { id: 'node-b', weight: 2 },
  { id: 'node-c', weight: 4 },
  { id: 'node-d', weight: 1 },
...
test('default hash produces fixed golden assignments', () => {
  ...
  for (const [key, single, replicas] of GOLDEN) {
    ...
    assert.deepEqual(
      router.selectReplicas(key, 3).map((n) => n.id),
      replicas,
      `selectReplicas(${JSON.stringify(key)}, 3)`,
    );
  }
});

test('results are independent of insertion order', () => {
  const orders: RendezvousNode[][] = [
    [...NODES],
    [...NODES].reverse(),
  ...
  for (const key of keys) {
    ...
    const replicas = orders.map((order) =>
      new RendezvousRouter(order).selectReplicas(key, 3).map((n) => n.id).join(','),
    );
...
test('batch APIs return results aligned to the input keys', () => {
  ...
  keys.forEach((key, i) =>
    assert.deepEqual(batches[i]!.map((n) => n.id), router.selectReplicas(key, 2).map((n) => n.id)),
  );
});
...
test('selection frequency is roughly proportional to weight', () => {
  const weighted: RendezvousNode[] = [
    { id: 'w1', weight: 1 },
    { id: 'w2', weight: 2 },
    { id: 'w4', weight: 4 },
  ];
  const router = new RendezvousRouter(weighted);
  const counts = new Map<string, number>(weighted.map((n) => [n.id, 0]));
  const total = 60_000;
  ...
  for (const [id, weight] of [['w1', 1], ['w2', 2], ['w4', 4]] as const) {
    const share = (counts.get(id) ?? 0) / total;
    const expected = weight / totalWeight;
    assert.ok(
      Math.abs(share - expected) < 0.04,
      `${id}: share ${share.toFixed(4)}, expected ~${expected.toFixed(4)}`,
    );
  }
});
 ...
 */
function tableHash(table: Record<string, bigint>): Hash64 {
  return (input: string) => table[input] ?? defaultHash64(input);
}

test('exactly equal scores are broken by ascending node id', () => {
  const tiedHash: Hash64 = (input: string) => {
...
test('score tie from equal weight/score is id-ordered regardless of insertion order', () => {
  const tiedHash: Hash64 = (input: string) =>
    input.startsWith('1:k') ? 42n : defaultHash64(input);
...
test('greedy domain rule: the globally best node wins its domain', () => {
  ...
  const hash: Hash64 = (input) => {
    if (input === frameKeyAndNodeId('k', 'x')) return 900n << 40n;
    if (input === frameKeyAndNodeId('k', 'y')) return 800n << 40n;
    if (input === frameKeyAndNodeId('k', 'z')) return 100n << 40n;
    return defaultHash64(input);
  };
  const router = new RendezvousRouter(
    [
...
test('replica order is descending score with id tie-break', () => {
  ...
  for (let i = 0; i < 200; i++) {
    ...
    for (let j = 1; j < picked.length; j++) {
      ...
      assert.ok(
        sPrev > sCur || (sPrev === sCur && prev.id < cur.id),
        `${key}: ${prev.id} before ${cur.id}`,
      );
    }
  }
});
...
test('removing the selected replica promotes the next-best compatible node only', () => {
  ...
  assert.equal(
    router.listNodes().find((n) => n.id === after.find((id) => id !== before[1]))?.labels?.[0],
    'a',
  );
});
...
test('readding the same node set restores every assignment', () => {
  ...
  for (const node of NODES) router.remove(node.id);
  for (const node of [...NODES].reverse()) router.add(node);
...
test('invalid nodes and arguments are rejected', () => {
  ...
  assert.throws(
    () => new RendezvousRouter([{ id: 'x', weight: 1, labels: ['ok', ''] }]),
    /non-empty string/,
  );
  const router = new RendezvousRouter();
...
test('splitmix-derived hash is stable across simulated separate processes', () => {
  ...
  for (const key of ['alpha', 'beta', 'gamma']) {
    assert.equal(r1.select(key)?.id, r2.select(key)?.id);
    assert.deepEqual(
      r1.selectReplicas(key, 2).map((n) => n.id),
      r2.selectReplicas(key, 2).map((n) => n.id),
    );
  }
...
test('large node set: heap path and label path agree on top-k logic', () => {
  ...
  const nodes: RendezvousNode[] = Array.from({ length: 400 }, (_, i) => ({
